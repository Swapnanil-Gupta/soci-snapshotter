...
parser.add_argument('-i', '--interval', type=int, default=1, help='Print interval in seconds')
parser.add_argument('-d', '--debug', action='store_true', help='Enable debug output')
parser.add_argument('-o', '--output', help='Output file for raw event data (JSON)')
...
# Dictionary to store operation statistics
stats = {}
all_events = []
...
def print_event(cpu, data, size):
    ...
    timestamp = datetime.fromtimestamp(event.ts / 1000000000).strftime('%H:%M:%S')
    ...
    # Print event details
    if args.debug:
        print(f"{timestamp} {event_record['comm']}[{event.pid}] {event_record['operation']} {event_record['path']} ({event.duration} ns)")
    
    # Update statistics
    op = event_record['operation']
    if op not in stats:
        stats[op] = {"count": 0, "total_time": 0, "min_time": float('inf'), "max_time": 0}
    stats[op]["count"] += 1
    stats[op]["total_time"] += event.duration
    stats[op]["min_time"] = min(stats[op]["min_time"], event.duration)
    stats[op]["max_time"] = max(stats[op]["max_time"], event.duration)
...
def print_stats():
    ...
    print(f"{'Operation':<10} {'Count':<10} {'Avg Time (ns)':<15} {'Min Time (ns)':<15} {'Max Time (ns)':<15}")
    print("-" * 70)
    ...
    for op, data in sorted(stats.items()):
        ...
        print(f"{op:<10} {data['count']:<10} {avg_time:<15.2f} {min_time:<15} {max_time:<15}")
